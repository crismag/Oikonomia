...
/** The parts of the organisation Administration opens one at a time. */
export type OrganizationPart = "campuses" | "people" | "ministries" | "groups" | "venues";

/** How many people the list shows before "Show more", so a church of hundreds
    does not render every row to find one. */
const PEOPLE_PAGE = 50;

export function OrganizationAdmin({ part }: { part: OrganizationPart }) {
  const organization = useOrganization();
  ...
  const [peopleShown, setPeopleShown] = useState(PEOPLE_PAGE);
  const needle = personQuery.trim().toLowerCase();
  const matchingPeople = needle
    ? organization.people.filter((person) =>
        [person.name, person.role, person.email]
          .filter(Boolean)
          .some((value) => String(value).toLowerCase().includes(needle)),
      )
    : organization.people;
  const listedPeople = matchingPeople.slice(0, peopleShown);
    ...
    <div className="space-y-4">
          ...
          <ul className="divide-y divide-border">
            {organization.campuses.map((campus) => (
              <li key={campus.id} className="px-4 py-2.5 text-[14px]">
                {campus.name}
                {campus.city ? (
                  <span className="text-muted-foreground"> · {campus.city}</span>
                ) : null}
              </li>
            ))}
            {organization.campuses.length === 0 ? (
              <li className="px-4 py-3 text-[13px] text-muted-foreground">
                No campuses yet. A campus is a place the church meets; ministries and people are
                filed under one.
              </li>
            ) : null}
          </ul>
      ...
      {part === "people" ? (
        <Section id="people" title="People" meta={`${organization.people.length}`}>
          ...
          {organization.people.length > PEOPLE_PAGE ? (
          ...
          <ul className="max-h-72 divide-y divide-border overflow-y-auto">
            ...
            {needle && matchingPeople.length === 0 ? (
              <li className="px-4 py-3 text-[13px] text-muted-foreground">
                Nobody here matches &ldquo;{personQuery.trim()}&rdquo;.
              </li>
            ) : null}
          </ul>
          {matchingPeople.length > listedPeople.length ? (
            <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border px-4 py-2.5 text-[12px] text-muted-foreground">
              <span>
                Showing {listedPeople.length} of {matchingPeople.length}
              </span>
              <Button
                type="button"
                variant="ghost"
                onClick={() => setPeopleShown((shown) => shown + PEOPLE_PAGE)}
              >
                Show more
              </Button>
            </div>
      ...
      {part === "groups" ? (
        <Section id="groups" title="Responsibility groups" meta={`${organization.groups.length}`}>
      ...
      {part === "venues" ? (
        <Section id="venues" title="Venues" meta={`${organization.venues.length}`}>
          <ul className="divide-y divide-border">
