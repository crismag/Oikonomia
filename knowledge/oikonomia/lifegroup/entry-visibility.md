---
id: lifegroup.entry-visibility
title: Who can read a gathering's notes
type: concept
category: lifegroup
summary: Each entry under Sharing and notes has its own audience — Leaders, This gathering's leaders, Selected viewers or Only me.
modules: [lifegroup]
pages: [gathering]
keywords: [sharing and notes, entry, prayer, concern, follow-up, visibility, audience, only me, private, held to a smaller audience]
aliases:
  - who can see my lifegroup notes
  - how do I add a private note to a gathering
  - what does held to a smaller audience mean
  - how do I record a prayer request
capabilities: []
hideWhen: []
related: [lifegroup.gathering-page, lifegroup.gathering-report, lifegroup.write-up]
destinations: [lifegroups]
---

**Sharing and notes** is where a gathering's leaders write down what was shared: prayer requests, concerns, follow-ups, visitors, decisions.

### Adding an entry

1. On the gathering, type in **Add an entry**.
2. Optionally pick a category, such as prayer, concern or follow-up.
3. Choose who can read it (it starts on **Leaders**).
4. Choose **Add**.

Only the gathering's assigned leaders can add entries. You can remove an entry you wrote.

### The audience choices

- **Leaders** — the ordinary choice. Anyone who can open the gathering can read it.
- **This gathering's leaders** — only the leaders assigned to this gathering.
- **Selected viewers** — only people named on the entry. This page has no way to name them, so an entry added here with this choice can be read only by you.
- **Only me** — only you.

Your church may rename these or add others; what each one allows is fixed.

The author can always read their own entry. When an entry is narrower than **Leaders**, it shows its audience with a lock.

### What others see

Someone who may not read an entry does not see its words anywhere — not on the page, not in the printable view. They see only a count, such as *2 entries are held to a smaller audience*.

### When an entry is the wrong place

Something that should reach one or two people — a concern about a family, for instance — is better written as a report from the gathering. See [Write a report from a gathering](topic:lifegroup.gathering-report).
