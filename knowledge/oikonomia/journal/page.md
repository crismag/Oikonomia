---
id: journal.page
title: Leadership Journal
type: page
category: leadership
summary: Private reflection and development; lines you choose can be copied into a separate leadership report.
modules: [journal]
pages: [leadership-journal]
keywords:
  [
    journal,
    leadership journal,
    reflection,
    development,
    private,
    summary,
    use in a summary,
    draw a summary,
  ]
aliases:
  - who can read my journal
  - can my bishop read my journal
  - how do I share a journal entry
  - how do I turn journal entries into a report
related: [reports.visibility, reports.create, permissions.capabilities]
destinations: [leadership-journal, leadership-reports]
---

The Leadership Journal is where you write for yourself: reflection, development, goals. It is under **Organization** in the sidebar, and everyone has one.

### Private to you

An entry is private to you. Nobody else can open it, whatever their role — administrators and senior leaders included.

### Writing

1. Choose **New entry**.
2. Give it a title and write. It saves as you go and shows **Saved**.

Entries are listed under **My development**. **Delete entry** asks first and cannot be undone.

### Sharing lines in a summary

There is no button that shares an entry. Instead:

1. In the entry, choose **Use in a summary**.
2. Tick the lines that belong in an accountability summary.
3. Choose **Draw a summary**.

The ticked lines are **copied** into a new leadership report, which starts private until you choose who may read it. Use **Open the summary** to do that. Everything you left unticked stays in the journal, and a reader of the report gains no access to the entry behind it.

See [Who can read a report](topic:reports.visibility).
